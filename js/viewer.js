...
function render(){
  ...
  if(game.phase==='briefing'){
    stateBox.innerHTML=`<span class="pill">${escapeHtml(c.title||'Справа')}</span>${c.packLabel?`<span class="pill">${escapeHtml(c.packLabel)}</span>`:''}<div class="caseBrief">${escapeHtml(c.publicBrief||'')}</div>${eventsHtml(c)}<div class="timerStage"><div><h2>Гравці читають секретні ролі</h2><div class="timerDots"><span></span><span></span><span></span></div></div></div>`;
    return;
  }
  if(game.phase==='performance'){
    stateBox.innerHTML=`<span class="pill">${escapeHtml(c.title||'Справа')}</span>${c.packLabel?`<span class="pill">${escapeHtml(c.packLabel)}</span>`:''}<div class="caseBrief">${escapeHtml(c.publicBrief||'')}</div>${eventsHtml(c)}<h2>Час виступів</h2><p class="muted">${escapeHtml(c.task||'')}</p>`;
    return;
  }
  if(game.phase==='voting'){
    const waiting=c.voteType==='judge_verdict'
      ? '<h2>Суддя готує вирок</h2><p class="muted">Фінальне рішення приймає ведучий-суддя.</p>'
      : `<h2>Голосування</h2><p class="muted">${players.filter(p=>p.vote_target).length}/${players.length} голосів</p>`;
    stateBox.innerHTML=`<span class="pill">${escapeHtml(c.title||'Справа')}</span>${c.packLabel?`<span class="pill">${escapeHtml(c.packLabel)}</span>`:''}<div class="caseBrief">${escapeHtml(c.publicBrief||'')}</div>${eventsHtml(c)}<div class="timerStage"><div>${waiting}<div class="timerDots"><span></span><span></span><span></span></div></div></div>`;
    return;
  }
