...
export function buildCase(mode,players,options={}){
  ...
  return buildCourtCase(players,options.courtPack||'mix',options.customCourtPack||null,options);
}

function buildCourtCase(players,packKey='mix',customPack=null,options={}){
  ...
  const accusation=pickUnused(pack.accusations,options.usedAccusations||[]);
  const evidence=pickUnused(pack.objects,options.usedEvidence||[]);
  ...
  jurors.forEach(p=>roles[p.id]={role:'Присяжний',instruction:'Слухай аргументи, став незручні питання й тисни на суддю. Остаточний вирок приймає ведучий-суддя.'});
  const allyPool=shuffled.filter(p=>p.id!==accused?.id&&p.id!==prosecutor?.id&&p.id!==defender?.id);
  const ally=(allyPool.length?allyPool:shuffled.filter(p=>p.id!==accused?.id))[0];
  if(ally){
    roles[ally.id]={
      role:'Таємний союзник',
      instruction:'Ти таємно допомагаєш підсудному. Прикривай його/її аргументи, плутай прокурора, але не кажи прямо, що ти союзник.'
    };
  }
  ...
  return {
    ...
    task:'Виступи по черзі: прокурор, адвокат, свідки, підсудний. Потім суддя-ведучий оголошує вирок.',
    voteType:'judge_verdict',
    voteOptions:[
...
function pickUnused(items,used=[]){
  ...
  const available=clean.filter(item=>!usedSet.has(String(item)));
